...
import { createStep } from "@/actions/registration";
...
export default function TambahAlurPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto pb-20">
      <div className="flex items-center justify-between mb-6">
        ...
        <h1 className="text-2xl font-serif font-bold text-gray-800">Tambah Tahapan</h1>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-4 gap-6">
             <div className="col-span-3">
                ...
                <input type="text" name="title" required className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-maroon-primary outline-none" placeholder="Contoh: Pengisian Formulir" />
             </div>
             <div className="col-span-1">
                <label className="block text-sm font-bold text-gray-700 mb-2">Urutan</label>
                <input type="number" name="order" required className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-maroon-primary outline-none text-center font-bold" />
             </div>
          </div>
          ...
          <div>
             ...
             <textarea 
                ...
                required 
                rows={3} 
                placeholder="Jelaskan apa yang harus dilakukan santri di tahap ini..."
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {icons.map((item, index) => (
                    <label key={item.val} className="cursor-pointer">
                        <input type="radio" name="icon" value={item.val} defaultChecked={index === 0} className="peer sr-only" />
          ...
          <div className="pt-4 border-t border-gray-100">
             <button type="submit" disabled={loading} className="w-full bg-maroon-primary text-white py-3 rounded-lg font-bold hover:bg-maroon-dark transition-all disabled:opacity-70 flex items-center justify-center gap-2">
                {loading ? <Loader2 className="animate-spin" size={20}/> : "Simpan Tahapan"}
             </button>
          </div>
        </form>
      </div>
