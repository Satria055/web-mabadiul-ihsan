...
export default function EditFacilityForm({ facility }: { facility: any }) {
  const router = useRouter();
  const { showAlert } = useAlert();
  const [loading, setLoading] = useState(false);
  const [preview, setPreview] = useState<string | null>(facility.image);
  ...
  return (
    <div className="max-w-2xl mx-auto pb-20">
      <div className="flex items-center justify-between mb-6">
        ...
        <h1 className="text-2xl font-serif font-bold text-gray-800">Edit Fasilitas</h1>
      </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-bold text-gray-700 mb-3 flex items-center gap-2">
                <ImageIcon size={18} /> Foto Fasilitas
            </label>
          ...
          <div>
             ...
             <input type="text" name="name" defaultValue={facility.name} required className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-maroon-primary outline-none" />
          </div>
          ...
          <div>
             ...
             <textarea name="description" defaultValue={facility.description} rows={4} className="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-maroon-primary outline-none"></textarea>
          </div>
